...
#the centrifuge reports don't have many things under phyla, somehow
#the k-reports don't have much more in terms of phyla really

entries = [] # a list
seen_list = [] # a list
number = len(file_list) 
count = 0 #keep track of what iteration of the loop we're on
trpeh = [] #total reads phyla exclude human, append this list as you go, each entry in the list is the trpeh for each file
...
for file_name in file_list:
	...
	for file_line in input_file: #loop over that list
		...
		if file_line[3] == "P": #ignore all lines that don't pass this
			...
			if taxid in seen_list: # if it's in seen list, it means we've seen it before, and the info from this file_line needs to be ammended to a line we've already made
				...
				new_entry = phyla_name + "\t" + taxid + "\t" + fill_in_line(reads,count)
...
program_nonnorm_output = open("stamp_input_multiple_files_nonnorm.tsv","w") #that won't get confusing...
...
for entry in entries:
	entry = entry.split("\t")
	if str(entry[1]) != "7711": #not chordates
...
program_output = open("stamp_input_multiple_files.tsv","w") #that won't get confusing...
